...
float OperationList::eval() const
{
	...
	for (auto &operation : opList)
	{
		if (operation.op == Operation::FORWARD || operation.op == Operation::FORWARD_DIAG)
		{
			...
			//  	distance = (float)operation.n * MAZE_1BLOCK_LENGTH / 2.0 * M_SQRT2;
			//  else
...
void OperationList::print()
{
	for (auto operation : opList)
	{
		...
		if (operation.op == Operation::TURN_LEFT90)
			printf("L");
		if (operation.op == Operation::TURN_RIGHT90)
			printf("R");
		if (operation.op == Operation::TURN_RIGHT45)
			printf("r");
		if (operation.op == Operation::TURN_LEFT45)
			printf("l");
		if (operation.op == Operation::FORWARD_DIAG)
			printf("D");
